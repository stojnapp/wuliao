...
const checkFormat={
  ...
  */
  ...
  /*根据时间戳判断一个时间是否在一个时间区间内,{默认包含起始日}*/
  checkedTimeStamp(beginstamp,endstamp,n){

    if(!beginstamp||!endstamp){
      Tools.TestTost('开始日期或结束日期不得为空!');
      return false;
    }

    let cha=endstamp-beginstamp;
    let ss=24*60*60*1000*n;
    if(cha<0){
      Tools.TestTost('开始日期不得大于结束日期!');
      return false;
    }
    if(cha>ss){
      Tools.TestTost(`只支持检索${n+1}天内数据!`);
      return false;
    }
    return true;
  },
